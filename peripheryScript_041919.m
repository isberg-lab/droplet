...
for j = 1:length(red_image_array)
    [mask, maskarea, redsum] = getMask_041919(green_image_array{j},...
        red_image_array{j});
    [periph_red, periph_pixel_num] = getPeriphery_041919(...
        red_image_array{j}, periphery_se_radius, mask);
    [cent_red, cent_pixel_num, centloc] = getCentroid_041919(...
        red_image_array{j}, centroid_se_radius, mask, periphery_se_radius);
    [periph_green, ~] = getPeriphery_041919(green_image_array{j},...
        periphery_se_radius, mask);
    graygreen = mat2gray(green_image_array{j});
    graygreen(~centloc) = 0;
    cent_green = sum(sum(graygreen));
